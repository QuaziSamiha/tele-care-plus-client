...
//*      const adapter = new CookieStorageAdapter<IConsent>("thp:consent", "thp:consent:change");
//*
//*   2. PASS THE INSTANCE TO useLocalStorageCollection(adapter).
...
//*   import { CookieStorageAdapter } from "@/lib/storage";
//*   import { useLocalStorageCollection } from "@/hooks/useLocalStorageCollection";
//*
//*   const adapter = new CookieStorageAdapter<ITheme>(
...
//*     const { items, isHydrated } = useLocalStorageCollection(adapter);
//*     ...
//*   };
//* ─────────────────────────────────────────────────────────────────────────────
...
export class CookieStorageAdapter<T> implements IStorageAdapter<T> {
  ...
  write(items: T[]): void {
    ...
    try {
      ...
      if (error instanceof DOMException) {
        console.warn(
          `[CookieStorageAdapter] Failed to write cookie for key "${this.key}". ` +
